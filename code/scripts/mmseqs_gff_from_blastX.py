...
for line in blastxfile:
	...
	if int(qstart) < int(qend):
		...
		strand = '-'
		newstop = int(start) + (int(qend)-1)
		newstart = int(start) + int(qstart) 

	attributes = "Dbxref="+args.source+":" + str(sseqid) + ";Note= pident:" + pident + ", mismatch:" + mismatch + ', gapopen:' + gapopen + ', evalue:' + evalue
	row = [qseqid, args.source, 'nucleotide_to_protein_match', str(newstart), str(newstop), str(bitscore), str(strand), str(qframe), attributes]
	row = "\t".join(row)
	f.write(row + '\n')

	# g.write('>' + qseqid + '; Db:' + args.source '; UnirefKB:' + sseqid + '\n')
	# g.write(SEQ + '\n')
