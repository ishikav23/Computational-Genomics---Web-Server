...
for line in blastxfile:
	...
	try:
		...
		qframe = str(qframe)
	attributes = "Dbxref=UnirefKB:" + str(sseqid) + ";Note= pident:" + pident + ", mismatch:" + mismatch + ', gapopen:' + gapopen + ', evalue:' + evalue + ', source:' + args.source
	row = [seqid, args.source, 'nucleotide_to_protein_match', str(newstart), str(newstop), str(bitscore), str(strand), str(qframe), attributes]
	row = '\t'.join(row)
	f.write(row + '\n')
